...
router.get('/',  check.LoginHome, function (req, res, next) {
    ...
    city.find({ email: req.session.user }, function (err, data) {
        ...
        if (req.query.error) {
            ...
            res.render('home', { viewErrorMsg: errorMsg,  page: 'Home', status: 'Login', login: email, name: firstname, city: infosCity });
            ...
            res.render('home', { viewSuccess: successMsg,  page: 'Home', status: 'Login', login: email, name: firstname, city: infosCity });
        } else {
            res.render('home', {page: 'Home', status: 'Login', login: email, name: firstname, city: infosCity });
        }
    }).sort({ _id: -1 });
});
...
router.post("/addCity",  check.LoginHome, function (req, res) {
    if (req.body.city) {
