...
router.get('/', check.LoginIndex, function (req, res, next) {
  if (req.query.error) {
    ...
    res.render("index", { viewErrorMsg: errorMsg, page: 'Login' });
  } else {
    res.render('index', { page: 'Login' });
  }
});

router.get('/signin',  check.LoginIndex, function (req, res, next) {
  if (req.query.error) {
    var errorMsg = "";
    switch (req.query.error) {
    ...
    res.render("signin", { viewErrorMsg: errorMsg, page: 'Register' });
  } else if (req.query.success) {
    var successMsg = "";
    switch (req.query.success) {
      case "1": successMsg = "Your account has been successfully created"; break;
    }
    res.render("signin", { viewSuccess: successMsg, page: 'Register' });
  } else {
    res.render('signin', { page: 'Register' });
  }
});

router.post("/signin", check.LoginIndex, function (req, res) {
  session = req.session;
...
router.post("/login", check.LoginIndex, function (req, res) {
  session = req.session;
